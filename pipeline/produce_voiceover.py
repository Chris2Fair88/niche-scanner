...
already-reviewed script, not creating new content, is the point. The
script can be either produce_script.py's own output (## heading +
**Voiceover:** lines) or a manually-written shot-list script structured
as a markdown table (Time | Visual | Voiceover | Caption | Notes, one
row per beat) -- extract_voiceover_lines() supports both.
...
resolve_niche_slug() directly rather than re-implementing it. --script-path
bypasses that resolution entirely for the input file, for scripts (like
manually-written ones) that don't live at scripts/<slug>-<run_date>.md.

Output: audio/<niche-slug>-<run_date>/ (or audio/<script filename stem>/
when --script-path is used) -- one .mp3 + one .alignment.json per beat,
plus a manifest.json tying beat order to files and durations. audio/ is
gitignored (generated binary media, same treatment as data/) -- the
alignment JSON is the one thing in there that's expensive to regenerate
(it costs an API call), which is exactly why it's saved alongside the
audio rather than discarded after use.
"""
...
def _split_table_row(row: str) -> list[str]:
    """Splits a markdown table row into cells on unescaped '|', dropping the
    row's own leading/trailing delimiter pipes and unescaping '\\|' back to
    a literal '|' within a cell."""
    row = row.strip()
    if row.startswith("|"):
        row = row[1:]
    if row.endswith("|"):
...
def parse_voiceover_table(script_md: str) -> dict[str, str] | None:
    """Extracts Voiceover lines from a markdown table (Time | Visual |
    Voiceover | Caption | Notes, one row per beat, in Hook/Setup/Turn/
    Payoff/CTA order) instead of produce_script.py's own '## heading' +
    '**Voiceover:**' structure -- covers manually-written shot-list
    scripts, like the origin-story one, that were never generated by
    produce_script.py and don't follow its format.

    Returns None if no table with a 'Voiceover' column is found at all, so
    a caller trying this as a fallback never mistakes an unrelated table
    for the voiceover one. Raises ValueError if such a table IS found but
    doesn't have exactly one data row per beat -- refusing to guess at a
    beat mapping is consistent with this module's existing "no
    partial/mismatched audio" stance, rather than silently pairing rows to
    the wrong beat.
    """
    lines_in = script_md.splitlines()
    header_idx = None
    voiceover_col = None
    for i, line in enumerate(lines_in):
        if "|" not in line or "voiceover" not in line.lower():
            continue
        if i + 1 >= len(lines_in) or not _TABLE_SEPARATOR_RE.match(lines_in[i + 1]):
            continue
        cells = _split_table_row(line)
        col = next((ci for ci, c in enumerate(cells) if c.lower().startswith("voiceover")), None)
        if col is not None:
            header_idx, voiceover_col = i, col
            break
    if header_idx is None:
        return None

    data_rows = []
    for line in lines_in[header_idx + 2:]:
        if not line.strip().startswith("|"):
            break
        data_rows.append(_split_table_row(line))

    if len(data_rows) != len(BEATS):
        raise ValueError(
            f"Found a Voiceover table with {len(data_rows)} data row(s) starting at line "
            f"{header_idx + 1}, but expected exactly {len(BEATS)} (one per beat, in order: "
            f"{BEATS}). Refusing to guess a beat mapping for a mismatched row count."
        )

    return {
        beat: (row[voiceover_col].strip() if voiceover_col < len(row) else "")
        for beat, row in zip(BEATS, data_rows)
    }


def extract_voiceover_lines(script_md: str) -> dict[str, str]:
    """Tries produce_script.py's own heading-based format first; if that
    doesn't yield all five beats, falls back to the markdown-table format
    (see parse_voiceover_table). Covers both without the caller needing to
    know in advance which structure a given script file uses."""
    heading_lines = parse_voiceover_lines(script_md)
    if len(heading_lines) == len(BEATS):
        return heading_lines
    table_lines = parse_voiceover_table(script_md)
    if table_lines:
        return table_lines
    return heading_lines
...
def run(run_date: str, topic: str | None, voice_id_override: str | None, script_path_override: str | None = None) -> str:
    ...
    if script_path_override:
        ...
        if not script_path.exists():
            log.error("No script found at %s.", script_path)
            sys.exit(1)
        niche_slug, niche_label = script_path.stem, None
        out_dir_name = script_path.stem
    else:
        ...
        if not script_path.exists():
            ...
            sys.exit(1)
        out_dir_name = f"{niche_slug}-{run_date}"

    script_md = script_path.read_text(encoding="utf-8")
    try:
        voiceover_lines = extract_voiceover_lines(script_md)
    except ValueError as e:
        log.error("Could not parse Voiceover lines from %s: %s", script_path, e)
        sys.exit(1)
    ...
    out_dir = common.AUDIO_DIR / out_dir_name
    ...
    manifest = {
        ...
        "run_date": None if script_path_override else run_date,
        "voice_id": voice_id,
        "model_id": model_id,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Explicit path to a markdown script file, bypassing niche+run-date resolution for the "
             "input file -- for scripts (e.g. manually-written ones) that don't live at "
             "scripts/<slug>-<run_date>.md. Output goes to audio/<script filename stem>/. Supports "
             "both produce_script.py's own '## heading' format and a markdown-table "
             "(Time | Visual | Voiceover | Caption | Notes) format.",
    )
    args = parser.parse_args()
    run(args.run_date, args.topic, args.voice_id, args.script_path)
